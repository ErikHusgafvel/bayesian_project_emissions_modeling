...
pooled_plotter_posterior_pred <- function() {
  ...
  hist(pooled_df$ypred, 
       ...
       col = "lightyellow",
       main="Plot X. \nPredictive distribution of the mean of the dataset")
}

pooled_plotter_posterior_pred()
```

```{r fig2, fig.height=4, fig.width=8, fig.align="center", echo=FALSE}
...
ggplot(hier_hist_df, aes(x = value)) +
  ...
  geom_vline(aes(xintercept=mean(hier_df$y_pred_new_county)),
             linetype = "dashed") +
  ggtitle("Plot X. \nPosterior predictive checks for selected countries.") +
        xlab("Means of CO2-emissions per capita") +
        ylab("") +
  theme(legend.position="top")
```

The predictive distribution of CO$_2$ per capita emission of the Kingdom of Saudi Arabia (SAU) is presented above. The mean of the predictive distribution is 11 with a standard deviation of 2.4, so Q95 is placed about at 15. If comparing this to the time-series figure in Section 2, the predicted value appears to be slightly too small. That's due to the fast increase in emissions which happened between 1950-1980. Our pooled model also counts values from that time interval, which causes too small values from the predictive distribution. Otherwise, the shape of the distribution looks as expected.

The mean of Finland's (FIN) predictive distribution is 8.7, which seems to be in the right place if you compare it to the development of previous years (Section 2 picture).  There is also some amount of uncertainty in distribution, so Q5 is at point 4.7 and Q95 at point 12.6. That's a moderately conservative distribution, and it's not possible to do accurate estimates based on this distribution. It still can give an estimate from which broad conclusions can be drawn about the level of emissions. For example, if we compare SAU's and FIN's predictive distribution, the higher level can be deduced, although accurate estimates could not be made.
