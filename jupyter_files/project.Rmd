...
```
```{r}
data_c02_population = data_co2*10^6/data_population

df_data4 <- data_c02_population[, (data_co2[dim(data_co2)[1], ]) >= 100]
df_data4 <- df_data4[,order(df_data4[69,])]
df_data4_2 <- data.frame(years=seq(1950,2018), df_data4)
df_plot4 <- melt(data = df_data4_2, id.vars = "years", variable.name = "country")
...
plot4 <- ggplot(df_plot4, aes(x=years, y=value, colour=country)) +
        geom_line() +
        ggtitle("co2 / population development big") +
        xlab("Year") +
        ylab("co2 per capita")
...
```{stan output.var=linearmodel}
// Hierarchical model
data {
      int<lower=0> N ;             // number of observations
      int<lower=0> J ;             // number of countries
      vector[N] x;                //  observation  years
...
transformed parameters {
      for (j in 1:J){
            vector[N] mu = alpha[j] + beta[j]*x;
      }
}
model {
      sigma ~ gamma(1,1);  // prior sd
      for (j in 1:J){
            alpha[j] ~ normal(0,27);
            beta[j] ~ normal(0,27);
      }
...
linear_data <- list(N = N,
                    J = 19,
                    x = seq(1950,2018),
                    y = data_c02_population)

pool_fit <- rstan::sampling(object = linearmodel,
                            data = linear_data,
                            iter = 2000,
                            warmup = 1000,
                            seed = SEED)
